...
//  segments.push_back(Segment_2(h3, h2));
//  segments.push_back(Segment_2(h2, h1));
//  segments.push_back(Segment_2(h1, h3));
  ...
  Point_2 query_point = h3;
  Halfedge_const_handle he = env.halfedges_begin();
  while (he->target()->point() != h3) {
    std::cout << " in while :[" << he->source()->point() << ", " << he->target()->point() << "]" << std::endl;
    he++;
    std::cout << " after :[" << he->source()->point() << ", " << he->target()->point() << "]" << std::endl;
  }
  std::cout << "final : [" << he->source()->point() << ", " << he->target()->point() << "]" << std::endl;
  //visibility query
