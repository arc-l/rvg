...
int main() {
  ...
  robot.moveTo(0, 0, 0);
  Polygon<T> robot1 = robot.rotateCopy(-5 * PI / 36);
  vertices = robot1.getVertices();
  Polygon<T> robot2 = robot.rotateCopy(-4 * PI / 36);
  ...
  std::cout << "Is polygonUpper simple? " << (polygonUpper.is_simple() ? "Yes" : "No") << std::endl;
...
//  polygonUpper.reverse_orientation();
  ...
  polygonUpper.reverse_orientation();
  Polygon_with_holes_2 sum2 = CGAL::minkowski_sum_by_reduced_convolution_2(polygonUpper, polygon1);
//  Polygon_with_holes_2 sum2 = CGAL::minkowski_sum_by_reduced_convolution_2(polygonLower, polygon1);
  ...
  Polygon_2 tmpPolygon = sum2Polygon;
  while (true) {
  ...
  first = curr = sum1Polygon.vertices_circulator();
  pythonScript += "sum1 = np.array([\n";
  do {
    std::cout << "x=" << curr->x() << "y=" << curr->y() << std::endl;
    pythonScript += "\t[" + std::to_string(CGAL::to_double(curr->x())) + ", " + std::to_string(CGAL::to_double(curr->y())) + "],\n";
