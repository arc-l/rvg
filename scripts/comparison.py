...
def main():
    ...
    df.replace("RVG", f"RVG ({mean_time:.2f}s)", inplace=True)
    ...
    ytick_start = 20
    ytick_end = 10 * (int((df.max()['Path Length'] / 10)))
    print(ytick_start, ytick_end)
    plt.yticks(np.arange(ytick_start, ytick_end + 1, 10))
    plt.ylim(ytick_start, ytick_end)
    plt.legend(loc='lower center', bbox_to_anchor=(0.5, -0.35), ncol=7)
    plt.tight_layout(rect=[0, 0.0, 1, 1])
    plt.savefig(f"{results_path}/comparison.png", dpi=500)
    plt.show()
